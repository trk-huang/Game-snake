...
class Grid(object):
    def __init__(self, master=None, height=16, width=24, offset=10, gridWidth=50, background="#808080"):
        self.height = height
        self.width = width
        self.offset = offset
        self.gridWidth = gridWidth
        self.background = background
        self.canvas = Canvas(master, width=self.width * self.gridWidth + 2 * self.offset,
                             height=self.height * self.gridWidth +
                                    2 * self.offset, bg=self.background)
    ...
    def draw(self, pos, color):
        ...
        self.canvas.create_rectangle(x, y, x + self.gridWidth, y + self.gridWidth, fill=color, outline=self.background)
...
class Snake(object):
    ...
    def move(self, new):
        ...
        pop = self.body.pop()
        self.grid.draw(pop, self.grid.background)
    ...
    def changeColor(self, new, color):
        self.color = color
        self.body.insert(0, new)
  ...
  def move(self, color="#EE82EE"):
    ...
    #吃到食物
      ...
      if self.food.type == 1:
        ...
        self.snake.cutDown(new)
      elif self.food.type == 4:
        self.snake.changeColor(new, color)
      else:
        self.snake.reset()
      self.display_food()
